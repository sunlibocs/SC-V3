...
class EdgeguidedNormalLoss(nn.Module):
    ...
    def forward(self, pred_depths, gt_depths, images, focal_length):
        ...
        for i in range(n):
            ...
            loss_cos10 = torch.log(1 + torch.exp(1 - input_cos))[
                mask_cos10 & consistency_mask
            ]
            loss_cos75 = torch.log(1 + torch.exp(input_cos))[
                mask_cos75 & consistency_mask
            ]
            if loss_cos10.shape[0] > 0:
                loss = loss + loss_cos10.mean()
            if loss_cos75.shape[0] > 0:
                loss = loss + loss_cos75.mean()

            # Regression for samples
            # mask_cos30_60 = (target_cos > self.cos_theta3) & (target_cos < self.cos_theta4)
            # loss += torch.sum(torch.abs(target_cos[mask_cos30_60 & consistency_mask] - input_cos[mask_cos30_60 & consistency_mask])) / (torch.sum(mask_cos30_60 & consistency_mask)+1e-8)
...
class EdgeguidedNormalRegressionLoss(nn.Module):
    ...
    def scale_shift_pred_depth(self, pred, gt):
        ...
        for i in range(b):
            ...
            A_i = torch.matmul(
                pred_valid_ones_i, pred_valid_ones_i.permute(1, 0)
            )  # [2, 2]
        ...
        pred_scale_shift = torch.matmul(
            pred_ones.permute(0, 2, 3, 1).reshape(b, h * w, 2), scale_shift_batch
        )  # [b, h*w, 1]
    ...
    def forward(self, pred_depths, gt_depths, images, focal_length):
        ...
        for i in range(n):
            ...
            mask_cos75 = target_cos < self.cos_theta1
            mask_cos10 = target_cos > self.cos_theta2
            # loss_cos10 = torch.log(1 + torch.exp(1-input_cos))[mask_cos10 & consistency_mask]
            # loss_cos75 = torch.log(1 + torch.exp(input_cos))[mask_cos75 & consistency_mask]
            # if loss_cos10.shape[0] > 0:
            #     loss = loss + loss_cos10.mean()
            # if loss_cos75.shape[0] > 0:
            #     loss = loss + loss_cos75.mean()
            ...
            loss += torch.sum(
                torch.abs(
                    target_cos[mask_cos75 & consistency_mask]
                    - input_cos[mask_cos75 & consistency_mask]
                )
            ) / (torch.sum(mask_cos75 & consistency_mask) + 1e-8)
            loss += torch.sum(
                torch.abs(
                    target_cos[mask_cos10 & consistency_mask]
                    - input_cos[mask_cos10 & consistency_mask]
                )
