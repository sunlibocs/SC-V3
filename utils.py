...
def save_checkpoint_stn(save_path, stn_state, epoch):
    torch.save(stn_state, save_path/'stn_{:02d}.pth'.format(epoch))


def save_checkpoint2(save_path, disp_state, pose_state, stn_state, is_best, filename='checkpoint.pth.tar'):
    file_prefixes = ['disp', 'pose', 'stn']
    states = [disp_state, pose_state, stn_state]
    for (prefix, state) in zip(file_prefixes, states):
        torch.save(state, save_path/'{}_{}'.format(prefix, filename))
